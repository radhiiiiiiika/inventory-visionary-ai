...
def detect_objects():
    try:
        ...
        # Find the most prominent item (highest count or confidence)
        max_item = max(detected_items.items(), key=lambda x: (x[1]['quantity'], x[1]['confidence']))
        item_name, item_data = max_item
